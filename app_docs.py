...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            user_doc_num = input('Введите номер документа - ')
            owner_name = get_doc_owner_name(user_doc_num)
            ...
            user_doc_num = input('Введите номер документа - ')
            directory_number = get_doc_shelf(user_doc_num)
            ...
            new_doc_shelf_num = input('Введите номер полки для хранения - ')
            new_doc_shelf_number = add_new_doc(new_doc_num, new_doc_type, new_doc_owner_name, new_doc_shelf_num)
            print('\nНа полку "{}" добавлен новый документ:'.format(new_doc_shelf_number))
        elif user_command == 'd':
            user_doc_num = input('Введите номер документа - ')
            doc_number, deleted = delete_doc(user_doc_num)
            if deleted:
                print('Документ с номером "{}" был успешно удален'.format(doc_number))
        elif user_command == 'm':
            user_doc_num = input('Введите номер документа - ')
            user_shelf_num = input('Введите номер полки для перемещения - ')
            move_doc_to_shelf(user_doc_num, user_shelf_num)
        elif user_command == 'as':
            shelf_num = input('Введите номер полки - ')
            shelf_number, added = add_new_shelf(shelf_num)
            if added:
